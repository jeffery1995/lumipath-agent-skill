...
**注意**: 使用 `localization_start` 时，通过 `dubbingVoiceId` 指定语音 ID
...
**参数**:
- `vid`: 视频 ID（从 video_list 获取）
- `sourceUrl`: 源视频 OSS URL
- `sourceLang`: 源语言代码（如 'zh'）
- `targetLang`: 目标语言代码（如 'en', 'ko'）
...
**参数**:
- `platforms`: 目标平台数组 [TIKTOK, YOUTUBE, INSTAGRAM]
- `text`: 帖子文案
- `mediaUrls`: 视频 URL 数组
...
**参数**:
- `connectionId`: 连接 ID（从 connections_list 获取）
